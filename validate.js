...
const PATTERNS_FILE = path.join(PATTERNS_DIR, "domain_patterns.json");
...
async function loadDomainPatterns() {
  try {
    ...
    log("info", "Domain patterns loaded", {
      count: domainPatterns.size,
      patterns,
    });
  } catch (error) {
    log("info", "No existing domain patterns file found, starting fresh");
  }
}

// Save patterns to file
async function saveDomainPatterns() {
  try {
    const patterns = Object.fromEntries(domainPatterns);
    await fs.writeFile(PATTERNS_FILE, JSON.stringify(patterns, null, 2));
    log("debug", "Domain patterns saved", {
      count: domainPatterns.size,
      patterns,
    });
  } catch (error) {
    log("error", "Failed to save domain patterns", { error: error.message });
  }
}

// Store pattern with persistence
function storePattern(domain, pattern) {
  const existingPattern = domainPatterns.get(domain);
  if (existingPattern !== pattern) {
    domainPatterns.set(domain, pattern);

    // Save to file asynchronously (don't await to avoid blocking)
    saveDomainPatterns().catch((err) => {
      log("error", "Failed to save pattern", {
        domain,
        pattern,
        error: err.message,
      });
    ...
    log("info", "Pattern stored", {
      domain,
      pattern,
      wasExisting: !!existingPattern,
    });
  }
}
    ...
    if (req.body.employees && req.body.domain) {
      ...
      domainPatternMap.forEach((pattern, emailDomain) => {
        if (!domainPatterns.has(emailDomain)) {
          storePattern(emailDomain, pattern);
        }
      });
...
// ... [Keep all your existing functions: fileStatus, downloadFile, extractEmailsFromCSV] ...
...
async function loadDomainPatterns() {
  try {
    ...
    log("info", "Domain patterns loaded", { count: domainPatterns.size });
  } catch (error) {
    log("info", "No existing domain patterns file found, starting fresh");
  }
}
...
async function saveDomainPatterns() {
  try {
    const patterns = Object.fromEntries(domainPatterns);
    await fs.writeFile(PATTERNS_FILE, JSON.stringify(patterns, null, 2));
    log("debug", "Domain patterns saved", { count: domainPatterns.size });
  } catch (error) {
    log("error", "Failed to save domain patterns", { error: error.message });
  }
}
...
// Update the pattern storage function
function storePattern(domain, pattern) {
  domainPatterns.set(domain, pattern);
  ...
  log("info", "Pattern stored", { domain, pattern });
}
